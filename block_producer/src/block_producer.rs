        ...
        match phase {
            ...
            Phase::Electra | Phase::Fulu | Phase::Gloas => {
                ...
                for attestation in attestations {
                    let converted = if phase >= Phase::Gloas {
                        let data = attestation.data;
                        let pre_pool = self
                            .producer_context
                            .attestation_agg_pool
                            .attestation_pre_pool_by_data(data)
                            .await;

                        match pre_pool {
                            Some(attestation_pre_pool) => {
                                let committee_index = attestation_pre_pool.committee_index;

                                match operation_pools::convert_to_electra_attestation_use_pre_pool(
                                    attestation,
                    ...
                    // Gloas path requires async pre-pool lookup, so conversion is done before filtering.
                    let Some((electra_attestation, committee_index)) = converted else {
                        continue;
                    };
