...
pub(crate) fn compute_proposer_index<P: Preset>(
    ...
    if state.is_post_eip7732() {
        ...
        // Convert PackedIndices to Vec<ValidatorIndex>
        let indices_vec: Vec<ValidatorIndex> = (0..indices.len())
            .map(|i| indices.get(i).expect("index in range"))
            .collect();
        ...
        .and_then(|selected| {
            selected.into_iter().next()
                .ok_or(Error::FailedToSelectProposer.into())
        })
