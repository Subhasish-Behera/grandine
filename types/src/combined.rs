...
impl<P: Preset> SszWrite for BeaconState<P> {
    fn write_variable(&self, bytes: &mut Vec<u8>) -> Result<(), WriteError> {
        match self {
            ...
            Self::Heze(state) => state.write_variable(bytes),
        }
    }
}
...
impl<P: Preset> SszHash for BeaconState<P> {
    ...
    fn hash_tree_root(&self) -> H256 {
        match self {
            ...
            Self::Heze(state) => state.hash_tree_root(),
        }
    }
}
...
impl<P: Preset> BeaconState<P> {
    ...
    pub fn set_cached_root(&self, root: H256) {
        match self {
            ...
            Self::Heze(state) => state.set_cached_root(root),
        }
    }
    ...
    pub fn deposit_requests_start_index(&self) -> Option<DepositIndex> {
        match self {
            ...
            Self::Heze(state) => Some(state.deposit_requests_start_index),
        }
    }
    ...
    pub fn from_ssz_at_phase(phase: Phase, bytes: &[u8]) -> Result<Self, ReadError> {
        let state = match phase {
            ...
            Phase::Heze => Self::Heze(SszReadDefault::from_ssz_default(bytes)?),
        };
...
impl<P: Preset> SszWrite for SignedBeaconBlock<P> {
    fn write_variable(&self, bytes: &mut Vec<u8>) -> Result<(), WriteError> {
        match self {
            ...
            Self::Heze(block) => block.write_variable(bytes),
        }
    }
}
...
impl<P: Preset> SszHash for SignedBeaconBlock<P> {
    ...
    fn hash_tree_root(&self) -> H256 {
        match self {
            ...
            Self::Heze(block) => block.hash_tree_root(),
        }
    }
}
...
impl<P: Preset> SignedBeaconBlock<P> {
    ...
    pub fn split(self) -> (BeaconBlock<P>, SignatureBytes) {
        match self {
            ...
            Self::Heze(block) => {
                let HezeSignedBeaconBlock { message, signature } = block;
                (message.into(), signature)
            }
        }
    }
    ...
    pub fn execution_payload(self) -> Option<ExecutionPayload<P>> {
        match self {
            Self::Phase0(_) | Self::Altair(_) | Self::Gloas(_) | Self::Heze(_) => None,
...
impl<P: Preset> BeaconBlock<P> {
    ...
    pub fn with_signature(self, signature: SignatureBytes) -> SignedBeaconBlock<P> {
        match self {
            ...
            Self::Heze(message) => HezeSignedBeaconBlock { message, signature }.into(),
        }
    }
    ...
    pub fn with_state_root(mut self, state_root: H256) -> Self {
        match &mut self {
            ...
            Self::Heze(block) => block.state_root = state_root,
        }
...
impl<P: Preset> From<BeaconBlock<P>> for SignedBeaconBlock<P> {
    fn from(beacon_block: BeaconBlock<P>) -> Self {
        match beacon_block {
            ...
            .into(),
            BeaconBlock::Heze(message) => HezeSignedBeaconBlock {
                message,
                signature: SignatureBytes::default(),
            }
            .into(),
...
impl<P: Preset> SszRead<Phase> for SignedBlindedBeaconBlock<P> {
    fn from_ssz_unchecked(phase: &Phase, bytes: &[u8]) -> Result<Self, ReadError> {
        let block = match phase {
            ...
            Phase::Heze => {
                return Err(ReadError::Custom {
                    message: "blinded block has slot in Heze",
                });
            }
        };
...
impl<P: Preset> SszRead<Phase> for ExecutionPayload<P> {
    fn from_ssz_unchecked(phase: &Phase, bytes: &[u8]) -> Result<Self, ReadError> {
        let block = match phase {
            ...
            Phase::Gloas | Phase::Heze => Self::Gloas(SszReadDefault::from_ssz_default(bytes)?),
        };
    ...
    pub const fn is_valid_with(&self, phase: Phase) -> bool {
        matches!(
            ...
            (Self::Bellatrix(_), Phase::Bellatrix)
                ...
                | (Self::Gloas(_), Phase::Gloas | Phase::Heze)
        )
    }
