...
use types::{
    ...
    traits::{BeaconState as _, PostAltairBeaconState, PostGloasBeaconState, SignedBeaconBlock as _},
};
...
pub struct Validator<P: Preset, W: Wait> {
    ...
    own_aggregators: BTreeMap<(AttestationData, CommitteeIndex), Vec<Aggregator>>,
...
impl<P: Preset, W: Wait + Sync> Validator<P, W> {
    ...
    async fn slot_head(&self, slot: Slot) -> Result<Result<SlotHead<P>, HeadFarBehind>> {
        ...
        let head_chain_link = head.clone();
        let beacon_state = {
            let controller = self.controller.clone_arc();

            tokio::task::spawn_blocking(move || {
                // Controller handles pre/post-Gloas state selection and advancing to `slot`.
                controller.proposer_head_state_at_slot(&head_chain_link, slot)
            })
            .await??
        };
                ...
                if let Some(blobs) = block_blobs {
                    if !blobs.is_empty() {
                            ...
                            .is_peerdas_activated()
                        {
                            ...
                            let data_column_sidecars = if block.phase() >= Phase::Gloas
                                && commitments_opt.is_none()
                            {
                                if self.validator_config.enable_payload_build && post_state_opt.is_some() {
                                    warn_with_peers!(
    ...
    async fn attest_and_start_aggregating(
        ...
        self.own_aggregators = own_singular_attestations
            ...
            .filter_map(|own_attestation| {
                ...
                Some(((data, committee_index), aggregator))
            })
    ...
    async fn publish_aggregates_and_proofs(&self, wait_group: &W, slot_head: &SlotHead<P>) {
        ...
        let (triples, proofs): (Vec<_>, Vec<_>) = self
            ...
            .map(|((data, committee_index), aggregators)| async move {
                ...
                let pre_pool = if phase >= Phase::Gloas {
                    let Some(pre_pool) = self
                        .attestation_agg_pool
                ...
                aggregators
                    ...
                    .filter_map(move |aggregator| {
                        ...
                        let aggregate_and_proof = if phase < Phase::Electra {
                            ...
                            let aggregate =
                                operation_pools::convert_to_electra_attestation(aggregate.clone())
                                    .ok()?;
                            ...
                            let aggregate = operation_pools::convert_to_electra_attestation_use_pre_pool(
                                aggregate.clone(),
                                pre_pool?,
                            )
                            .ok()?;
