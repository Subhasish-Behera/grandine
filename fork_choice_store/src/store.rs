...
use crate::{
    ...
    PayloadAttestationItem, PayloadAttestationOrigin, PayloadAttestationValidationError,
...
impl<P: Preset, S: Storage<P>> Store<P, S> {
    ...
    /// Get ChainLink from FULL-variant map, or finalized.
    /// Returns None if block only exists as unfinalized EMPTY variant.
    ///
    /// safe to call as the any block emtpty or full creates unfinalized_locations_full
    //. when called to be used for envelope_state(), the caller should handle payload arriveed vs not arrived(like using is_some). 
    /// dev notes: it included finalized blocks
    /// Note: Finalized blocks are included regardless of which variant was finalized,
    /// since we don't track variant distinction after finalization. For strict
    /// FULL-only semantics on unfinalized blocks, check unfinalized_locations_full directly.
    #[must_use]
    ...
    /// Uses map membership to correctly distinguish EMPTY vs FULL placeholder vs FULL.
    ...
    /// dev notes: Uses Local view and not what the node is supposed to be like(which is done at get_parent_payload_status using bid info with a internt to create/enforce a Parent Local view) 
    /// - though the callers at head calculation use/call it even for pre-gloas and it works but it can be  confusiong given pre gloas is
    /// is in unfnalized_locations_full. should guard those callsite with gloas phase chec
    #[must_use]
    fn node_payload_status_at(&self, location: Location) -> ForkChoicePayloadStatus {
        ...
        // FULL variant - check bid existence (pre-Gloas / no bid → Empty)
        let has_bid = chain_link.block.message().body().with_payload_bid().is_some();
        if !has_bid {
            //pre gloas block if called
            return ForkChoicePayloadStatus::Empty;
        }
    ...
    pub fn block_slot(&self, block_root: H256) -> Option<Slot> {
        self.chain_link_full(block_root)
            .map(|chain_link: &ChainLink<P>| chain_link.block.message().slot())
    }
    ...
    /// TBD: clarification on retrieve_column_sidecars_and_kzg_commitments function acc to spec.
    #[must_use]
    ...
    fn is_head_weak(&self, head_root: H256) -> bool {
        ...
        if let Some(head_state) = &chain_link.block_state {
            let head_slot = chain_link.block.message().slot();
    ...
    /// [`is_parent_strong`](https://github.com/ethereum/consensus-specs/blob/v1.7.0-alpha.1/specs/phase0/fork-choice.md#is_parent_strong)
    ///
    /// Check if a parent block is strong (has more than REORG_PARENT_WEIGHT_THRESHOLD% of committee weight).
    /// Used to ensure votes aren't being hoarded before performing a reorg.
    ///
    /// Per spec, uses get_parent_payload_status to determine variant (differs from is_head_weak).
    #[allow(dead_code)]
    fn is_parent_strong(&self, parent_root: H256) -> bool {
        let committee_weight = self.committee_weight();
        let parent_threshold = committee_weight * self.chain_config.reorg_parent_weight_threshold / 100;

        // Per spec: parent_payload_status = get_parent_payload_status(store, block)
        // For now, use FULL placeholder balance (same as is_head_weak)
        let parent_weight = self.unfinalized_locations_full
            .get(&parent_root)
            .map(|loc| self.unfinalized[&loc.segment_id][loc.position].attesting_balance)
            .unwrap_or(0);
    ...
    fn should_apply_proposer_boost(&self) -> bool {
        ...
        let parent_slot = parent_chain_link.block.message().slot();
    ...
    pub fn validate_block_with_custom_state_transition(
        ...
        //  consensus parent block must be known
        let Some(parent) = self.chain_link_full(block.message().parent_root()) else {
            return Ok(BlockAction::DelayUntilParent(block.clone_arc()));
        };

        // ePBS: If block claims to build on parent's FULL variant,
        // check that parent's execution payload exists in execution_payload_locations
        // note: most likely will be added in the spec as a validation rule for beacon block
        if let Some(gloas_body) = block.message().body().with_payload_bid() {
            let parent_block_hash = gloas_body
    ...
    pub fn validate_execution_payload_envelope_with_state(
        ...
        if origin.verify_signatures() {
            ...
            let pubkey = if builder_index == BUILDER_INDEX_SELF_BUILD {
                ...
                // dev notes: delted the previouis code as it was looking for builder index at accessors::public_key
    ...
    fn insert_block(&mut self, chain_link: ChainLink<P>) -> Result<()> {
        ...
        let parent = if let Some(gloas_body) = block.message().body().with_payload_bid() {
            ...
            if !builds_on_full {
                ...
                // FULL: lookup via execution_payload_locations using bid.parent_block_hash
                let parent_block_hash = gloas_body.signed_execution_payload_bid().message.parent_block_hash;
                let full_parent = self.execution_payload_locations.get(&parent_block_hash).copied();

                // Defensive: if builds_on_full but payload not found, fall back to FULL placeholder
                if full_parent.is_none() {
    ...
    fn should_prune_segment(
        ...
        // Parent segment may have been pruned earlier in this iteration.
        // If so, this child is also orphaned and should be pruned.
        if !self.unfinalized.contains_key(&parent.segment_id) {
            return true;
        }
    ...
    fn extend_latest_messages_after_finalization(&mut self) {
        ...
        self.latest_messages.extend(core::iter::repeat_n(None, added));
    }
        ...
        for valid_attestation in valid_attestations {
            ...
            for validator_index in attesting_indices {
                ...
                if let Some(Some(old_message)) = &self.latest_messages.get(index) {
                    ...
                    // Pre-Gloas: payload_present always false, so this is equivalent to old behavior
    ...
    fn should_follow_branch_point(
        ...
        let (sib_bal, br_bal) = (sibling_score.0, branch_point.score.0);

        if sib_bal < br_bal || sibling.is_invalid() {
            return true;
        }
    ...
    fn should_choose_next_branch_point(
        ...
        if next_root == best_root && self.phase() >= Phase::Gloas {
            ...
            let next_tb = self.get_payload_status_tiebreaker(next_root, next_status);
            let best_tb = self.get_payload_status_tiebreaker(best_root, best_status);
            return (next_tb, next_root) > (best_tb, best_root);
        }
  ...
  //
  // Baseline winner:
  // - `best_descendant_of_segment` starts as sibling/same-segment continuation when viable:
  //   `viable.then_some(*segment_id)`.
  // - This means sibling wins by default unless a branch-group candidate beats it.
  ...
  //(note: keeps the previous approch of checking is_invalid for sinbling and not inditvidual branch point which happens though is_viabe for segments)
  // Group reduction rules:
  //(had to separate the functions to not bloat the core logic and intrdouce tiebreaking ruler though extrenal fucntions)
  // 1) If no running winner, start group with `next_branch`.
  // 2) If `next_branch.parent == current_best.parent`, reduce inside the same parent-group
  //    using `should_choose_next_branch_point(current_best, next_branch)`.
  //    This is branch-vs-branch selection (viability/score/tiebreak), not sibling comparison.
  // 3) If parent changes, close previous group:
  //    - compare that group winner once vs sibling via `should_follow_branch_point`,(
  //.     (major change: sibling is no more the base line comparision for every popped out branch point, oonly happnes once per segment)
  //    - update `best_descendant_of_segment` only if branch_point wins,
  //    - then start new group with `next_branch`.
  //
  // Final group flush:
  // - After loop exits, one parent-group may still be pending in `best_branch_for_parent`.
  // - Flush it with the same sibling comparison path.
  //
  // Why this structure:
  ...
  // dev notes: infact I wasn't sure how the previous implemention pased certain cases like lighter branchpoint for a segment popped later than heavier branchpoint of same segment. 
  //(maybe was working on some assumption/rule/optimization on number of branchpoints which is changed by atleast 1 by empty/full design )
  ...
  //   parent comparison in higher segments (or used as root-segment candidate at top).
    ...
    pub fn invalidate_block_and_descendant_payloads2(&mut self, block_root: H256) {
        ...
        while i < to_invalidate.len() {
            ...
            let Some(segment) = self.unfinalized.get(&seg_id) else {
                continue;
            };
            ...
            for (child_seg_id, child_segment) in self.unfinalized.iter() {
                if processed_segments.contains(child_seg_id) {
                    continue;
                }
